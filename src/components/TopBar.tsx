...
export function TopBar({ table, view, serviceCount, language, elderly, theme, onToggleLanguage, onToggleElderly, onToggleTheme, onView, onService, onConsole }: TopBarProps) {
  ...
  return (
    <>
      ...
      <header className="sticky top-0 z-30 border-b border-charcoal-900/5 bg-rice-50/95 backdrop-blur-xl dark:border-rice-100/10 dark:bg-charcoal-700/95">
        <div className="mx-auto flex h-16 max-w-7xl items-center gap-3 px-4 lg:px-6">
          <button onClick={() => onView('menu')} className="flex items-center gap-2 text-left">
            <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-chili-500 text-lg font-black text-white shadow-md dark:bg-chili-600">{t('common.brand')}</span>
            <span className="hidden sm:block"><strong className="block leading-4 text-charcoal-900 dark:text-rice-50">{t('common.brand_name')}</strong><small className="text-charcoal-500 dark:text-rice-200/75">{t('common.subtitle')}</small></span>
          </button>
          <span className="ml-1 flex items-center gap-1 rounded-full bg-rice-200 px-3 py-2 text-xs font-bold text-charcoal-700 dark:bg-rice-100/10 dark:text-rice-200"><MapPin size={13} className="text-chili-500 dark:text-chili-100" />{tableLabel}</span>
          ...
          <Dialog>
            ...
            <DialogContent title={t('common.member_title')}>
              ...
              <div className="mt-4 grid grid-cols-2 gap-3">
                <div className="rounded-2xl bg-white p-4 shadow-sm dark:bg-charcoal-900/60"><p className="text-xs text-charcoal-500 dark:text-rice-200/75">{t('common.queue')}</p><p className="mt-2 text-2xl font-extrabold text-charcoal-900 dark:text-rice-50">A018</p><p className="text-xs text-chili-500 dark:text-chili-100">{t('common.queue_ahead')}</p></div>
                <div className="rounded-2xl bg-white p-4 shadow-sm dark:bg-charcoal-900/60"><p className="text-xs text-charcoal-500 dark:text-rice-200/75">{t('common.benefits')}</p><p className="mt-2 text-2xl font-extrabold text-charcoal-900 dark:text-rice-50">4 <small className="text-sm">{t('common.tickets')}</small></p><p className="text-xs text-amber-500 dark:text-amber-400">{t('common.coupon')}</p></div>
              </div>
            </DialogContent>
          </Dialog>
          <Button variant="outline" size="icon" onClick={onConsole} aria-label={t('common.aria_console')}><LayoutDashboard size={18} /></Button>
          <Button variant="outline" size="icon" onClick={onToggleTheme} aria-label={theme === 'dark' ? t('common.aria_theme_dark') : t('common.aria_theme_light')}>
            {theme === 'dark' ? <Sun size={18} className="text-amber-400" /> : <Moon size={18} />}
          </Button>
          <Button variant="outline" size="icon" onClick={onToggleElderly} aria-label={elderly ? '切换至常规模式' : '切换至老人模式'}>
            <Accessibility size={18} className={elderly ? 'text-chili-500 dark:text-chili-100' : ''} />
          </Button>
