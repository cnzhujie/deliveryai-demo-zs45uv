...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-rice-100 paper-noise dark:bg-charcoal-900">
        ...
        <main className="mx-auto grid max-w-7xl gap-6 px-4 py-5 pb-28 lg:grid-cols-3 lg:px-6 lg:py-7 lg:pb-8">
          ...
          <aside className="hidden lg:block">
            <div className="sticky top-28">
              ...
              <div className="mt-4 rounded-2xl border border-amber-400/30 bg-amber-100/70 p-4 text-sm text-charcoal-700 dark:border-amber-400/25 dark:bg-amber-400/10 dark:text-rice-200">
                <p className="font-bold">{t('common.collab_title')}</p>
                <p className="mt-1 leading-6 text-charcoal-500 dark:text-rice-200/75">{t('common.collab_desc')}</p>
              </div>
            </div>
          </aside>
        </main>
      ...
      <nav className="safe-bottom fixed bottom-0 left-0 right-0 z-30 grid grid-cols-4 border-t border-charcoal-900/5 bg-white/95 px-2 pt-2 backdrop-blur lg:hidden dark:border-rice-100/10 dark:bg-charcoal-700/95">
      ...
      <div className="pointer-events-none fixed left-1/2 top-24 z-40 -translate-x-1/2 rounded-full bg-charcoal-900/90 px-4 py-2 text-xs font-semibold text-white shadow-float dark:bg-rice-100/15 dark:text-rice-50">
        {state.lastMessage}
      </div>
    </div>
  )
...
function MobileNav({ active, icon: Icon, label, badge, onClick }: { active: boolean; icon: typeof MenuIcon; label: string; badge?: number; onClick: () => void }) {
  return (
    <button onClick={onClick} className={`relative flex flex-col items-center gap-1 rounded-xl py-2 text-xs font-semibold transition ${active ? 'bg-chili-50 text-chili-500 dark:bg-chili-500/20 dark:text-chili-100' : 'text-charcoal-500 dark:text-rice-200/75'}`}>
